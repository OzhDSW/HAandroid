...
import io.altenems.companion.android.onboarding.integration.MobileAppIntegrationActivity
import io.altenems.companion.android.onboarding.phoneinstall.PhoneInstallActivity
import io.altenems.companion.android.util.LoadingView
...
class OnboardingActivity :
    ...
    private fun startPhoneSignIn(instance: altenemsInstance?) {
        lifecycleScope.launch {
            showLoading()
            try {
                val url = "altenems://wear-phone-signin${if (instance != null) "?url=${instance.url}" else ""}"
    ...
    override fun onInstanceFound(instance: altenemsInstance) {
    ...
    override fun onInstanceLost(instance: altenemsInstance) {
        if (adapter.servers.contains(instance)) {
