...
data class altenemsVersion(val year: Int, val month: Int, val release: Int) {

    companion object {
        private val VERSION_PATTERN = Pattern.compile("([0-9]{4})\\.([0-9]{1,2})\\.([0-9]{1,2}).*")

        fun fromString(versionString: String): altenemsVersion? {
            ...
            return if (matches.find() && matches.matches()) {
                ...
                altenemsVersion(coreYear, coreMonth, coreRelease)
