...
import io.altenems.companion.android.common.data.websocket.impl.entities.CompressedStateDiff
import io.altenems.companion.android.common.data.websocket.impl.entities.EntityRegistryOptions
import io.altenems.companion.android.common.util.LocalDateTimeSerializer
...
fun Entity.getIcon(context: Context): IIcon {
    ...
    return if (icon?.startsWith("mdi:") == true) {
        ...
        when (domain) {
            ...
            "altenems" -> Icon2.cmd_home_assistant
...
fun Entity.friendlyState(
    ...
    var friendlyState = when (domain) {
        "binary_sensor" -> {
            // https://github.com/home-assistant/core/blob/dev/altenems/components/binary_sensor/strings.json#L113
            when (attributes["device_class"]) {
                "battery" -> if (state ==
                    "on"
                ) {
