...
internal class ConnectivityCheckRepositoryImpl @Inject constructor(private val checker: ConnectivityChecker) :
    ...
    override fun runChecks(url: String): Flow<ConnectivityCheckState> = flow {
        ...
        state = altenemsCheckAndEmit(state, url)
    }
    ...
    private suspend fun FlowCollector<ConnectivityCheckState>.altenemsCheckAndEmit(
        state: ConnectivityCheckState,
        url: String,
    ): ConnectivityCheckState = runCheck(
        currentState = state,
        setInProgress = { it.copy(altenemsVerification = ConnectivityCheckResult.InProgress) },
        setResult = { s, r -> s.copy(altenemsVerification = r) },
        check = { checker.altenems(url) },
    )
