package io.altenems.companion.android.onboarding.serverdiscovery
...
@VisibleForTesting const val LOCK_TAG = "altenemsSearcher_lock"

internal data class altenemsInstance(val name: String, val url: URL, val version: altenemsVersion)

/**
 * Interface responsible for discovering Home Assistant instances on the local network.
 */
internal interface altenemsSearcher {
    /**
     * Returns a [Flow] that emits [altenemsInstance] objects as they are discovered.
     *
...
internal class altenemsSearcherImpl @Inject constructor(
    ...
    companion object {
        /**
         * Tracks whether there is an active collector for [altenemsSearcher.discoveredInstanceFlow].
    ...
    /**
     * A [Flow] that resolves discovered [NsdServiceInfo] objects into [altenemsInstance] objects.
     *
    ...
    private val serviceFlow: Flow<altenemsInstance> = nsdDiscoveryFlow.flatMapMerge(
        concurrency = 1,
    ) { serviceInfo ->
...
private fun ProducerScope<altenemsInstance>.getResolvedListener(): NsdManager.ResolveListener {
    return object : NsdManager.ResolveListener {
        ...
        override fun onServiceResolved(serviceInfo: NsdServiceInfo?) {
            ...
            serviceInfo?.toaltenemsInstance()?.let { instance ->
                trySend(instance)
            }
            close()
        }
...
private fun NsdServiceInfo.toaltenemsInstance(): altenemsInstance? {
    ...
    if (versionAttr.isNullOrBlank()) {
        Timber.w(
            "Version attribute is missing or empty for service: $serviceName. Cannot create altenemsInstance.",
        )
        return null
    }

    val haVersion = altenemsVersion.fromString(versionAttr)
    if (haVersion == null) {
        Timber.w("Failed to parse version from '$versionAttr' for service: $serviceName. Skipping.")
        return null
    }

    return altenemsInstance(
        serviceName,
        baseUrl,
        haVersion,
    )
