...
import io.altenems.companion.android.WIPFeature
...
import io.altenems.companion.android.common.compose.theme.HATheme
import io.altenems.companion.android.launch.applock.HazeLockOverlay
...
import io.altenems.companion.android.util.PLAY_SERVICES_FLAVOR_DOC_URL
import io.altenems.companion.android.util.PlayServicesAvailability
import io.altenems.companion.android.util.compose.HAApp
 ...
 */
private const val LOCK_SCREEN_ALIAS_CLASS = "io.altenems.companion.android.launch.LaunchOverLockScreen"
...
//        data class OpenWearOnboarding(val wearName: String, val urlToOnboard: String?) : DeepLink
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            HATheme {
                ...
                val wireGuardEvent by viewModel.wireGuardEvents.collectAsStateWithLifecycle()
                ...
                LaunchedEffect(wireGuardEvent) {
                    wireGuardEvent?.let { event ->
                        if (event is LaunchViewModel.WireGuardEvent.Connect) {
                            if (wgManager.prepareVpn(this@LaunchActivity, VPN_PERMISSION_CODE)) {
                                wgManager.connect(event.fileName, event.configText)
                            }
                        }
                    }
                }
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == VPN_PERMISSION_CODE && resultCode == RESULT_OK) {
            val event = viewModel.wireGuardEvents.value
            if (event is LaunchViewModel.WireGuardEvent.Connect) {
                wgManager.connect(event.fileName, event.configText)
            }
        }
