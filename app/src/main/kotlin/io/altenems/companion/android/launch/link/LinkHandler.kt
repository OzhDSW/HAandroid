...
private const val DEEP_LINK_SCHEME = "altenems"
...
/**
 * Handles universal links from `https://my.home-assistant.io` and some of the deep links from `altenems://`
 */
interface LinkHandler {
    /**
     * Processes the given [uri] from `https://my.home-assistant.io` or `altenems://` and determines the
...
class LinkHandlerImpl @Inject constructor(private val serverManager: ServerManager) : LinkHandler {
     ...
     * Universal Link: `https://my.home-assistant.io/invite#url=http://altenems.local:8123`
     * Deep Link: `altenems://invite/#url=http://altenems.local:8123`
     *
    ...
    private fun handleInviteLink(uri: Uri): LinkDestination {
        // Extract the server URL from the fragment #url=http://altenems.local:8123 and make it queryParam in a fake URI
    ...
    /**
     * Handles navigate deep links from `altenems://navigate/...`.
     *
