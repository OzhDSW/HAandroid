...
class WearOnboardingListener : WearableListenerService() {
    ...
    override fun onMessageReceived(event: MessageEvent) {
        ...
        if (event.path == "/request_home_assistant_instance") {
            ...
            sendaltenemsInstance(nodeId)
        }
    }

    // Uses runBlocking because WearableListenerService may destroy the service once onMessageReceived returns
    private fun sendaltenemsInstance(nodeId: String) = runBlocking {
        Timber.d("sendaltenemsInstance: $nodeId")
        ...
        if (url != null) {
            ...
            try {
                Wearable.getDataClient(this@WearOnboardingListener)
                    ...
                    .await()
                Timber.d("sendaltenemsInstance: success")
            } catch (e: Exception) {
