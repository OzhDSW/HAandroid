...
class SettingsWearViewModel @Inject constructor(
    ...
    override fun onCleared() {
        try {
            Wearable.getDataClient(getApplication<AltenEmsApplication>()).removeListener(this)
        } catch (e: Exception) {
    ...
    fun sendHomeFavorites(favoritesList: List<String>) = viewModelScope.launch {
        val application = getApplication<AltenEmsApplication>()
    ...
    private fun readUriData(uri: String?): ByteArray {
        ...
        return getApplication<AltenEmsApplication>().contentResolver.openInputStream(
            uri.toUri(),
        )!!.buffered().use {
            it.readBytes()
        }
    ...
    fun sendAuthToWear(
        ...
        val app = getApplication<AltenEmsApplication>()
        try {
    ...
    fun sendTemplateTileInfo() {
        ...
        try {
            Wearable.getDataClient(getApplication<AltenEmsApplication>())
    ...
    private fun watchConnectionError(app: AltenEmsApplication) {
        viewModelScope.launch {
