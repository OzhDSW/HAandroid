package io.altenems.companion.android.frontend
...
import io.altenems.companion.android.common.data.connectivity.ConnectivityCheckRepository
import io.altenems.companion.android.common.data.connectivity.ConnectivityCheckResult
...
import io.altenems.companion.android.common.data.prefs.ScreenOrientation
import io.altenems.companion.android.common.data.prefs.ZoomSettings
...
import io.altenems.companion.android.frontend.auth.HttpAuthManager
import io.altenems.companion.android.frontend.dialog.FrontendDialog
import io.altenems.companion.android.frontend.dialog.FrontendDialogManager
...
import io.altenems.companion.android.frontend.exoplayer.ExoPlayerUiState
import io.altenems.companion.android.frontend.exoplayer.FrontendExoPlayerManager
...
import io.altenems.companion.android.frontend.externalbus.outgoing.SuccessResultMessage
import io.altenems.companion.android.frontend.filechooser.FileChooserManager
import io.altenems.companion.android.frontend.gesture.FrontendGestureHandler
import io.altenems.companion.android.frontend.gesture.GestureResult
import io.altenems.companion.android.frontend.handler.FrontendBusObserver
import io.altenems.companion.android.frontend.handler.FrontendHandlerEvent
import io.altenems.companion.android.frontend.js.FrontendJsBridgeFactory
import io.altenems.companion.android.frontend.navigation.FrontendEvent
import io.altenems.companion.android.frontend.permissions.PermissionManager
import io.altenems.companion.android.frontend.url.FrontendUrlManager
import io.altenems.companion.android.frontend.url.UrlLoadResult
import io.altenems.companion.android.testing.unit.FakeClock
...
class FrontendViewModelTest {
    ...
    inner class ErrorFlow {
        ...
        fun `Given loading state when onWebViewCreationFailed called then state transitions to Error with WebViewCreationError`() = runTest {
            ...
            assertEquals(io.altenems.companion.android.common.R.string.webview_creation_failed, error.message)
    ...
    inner class MessageHandling {
        ...
        fun `Given auth error message result when collected then state transitions to Error`() = runTest {
            ...
            val authError = FrontendConnectionError.AuthenticationError(
                message = io.altenems.companion.android.common.R.string.error_connection_failed,
                ...
                messageFlow.emit(
                    FrontendHandlerEvent.EntityAddToExecuted(
                        FrontendEvent.NavigateToWidgetConfig(
                            entityId = "light.test",
                            widgetType = io.altenems.companion.android.frontend.navigation.WidgetType.Entity,
                        ),
                    ),
                )
        ...
        fun `Given connectivity checks running when repository emits state then viewModel updates state`() = runTest {
            ...
            val completedState = ConnectivityCheckState(
                dnsResolution = ConnectivityCheckResult.Success(
                    messageResId = io.altenems.companion.android.common.R.string.connection_check_dns,
                    details = "192.168.1.1",
                ),
                portReachability = ConnectivityCheckResult.Success(
                    messageResId = io.altenems.companion.android.common.R.string.connection_check_port,
                    details = "8123",
                ),
                tlsCertificate = ConnectivityCheckResult.NotApplicable(
                    messageResId = io.altenems.companion.android.common.R.string.connection_check_tls_not_applicable,
                ),
                serverConnection = ConnectivityCheckResult.Success(
                    messageResId = io.altenems.companion.android.common.R.string.connection_check_server_success,
                ),
                altenemsVerification = ConnectivityCheckResult.Success(
                    messageResId = io.altenems.companion.android.common.R.string.connection_check_home_assistant_success,
                ),
            )
        ...
        fun `Given connectivity checks with failure when repository emits state then viewModel has failure state`() = runTest {
            val failedState = ConnectivityCheckState(
                dnsResolution = ConnectivityCheckResult.Failure(
                    messageResId = io.altenems.companion.android.common.R.string.connection_check_error_dns,
                ),
                portReachability = ConnectivityCheckResult.Failure(
                    messageResId = io.altenems.companion.android.common.R.string.connection_check_skipped,
                ),
                ...
                altenemsVerification = ConnectivityCheckResult.Failure(
                    messageResId = io.altenems.companion.android.common.R.string.connection_check_skipped,
                ),
            )
