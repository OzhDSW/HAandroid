...
class altenemsSearcherImplTest {

    private lateinit var nsdManager: NsdManager
    private lateinit var wifiManager: WifiManager
    private lateinit var searcher: altenemsSearcherImpl

    @BeforeEach
    fun setup() {
        nsdManager = mockk(relaxed = true)
        wifiManager = mockk(relaxed = true)
        searcher = altenemsSearcherImpl(nsdManager, wifiManager)
        // Reset the singleton for each test
        altenemsSearcherImpl.hasCollector.set(false)
    }
    ...
    fun `Given one discoverable instance when discoveredInstanceFlow is called then should emit instance`() = runTest {
        ...
        discoveredInstanceFlow {
            ...
            val item = awaitItem()
            assertTrue(altenemsSearcherImpl.hasCollector.get())
            assertEquals(serviceInfo.serviceName, item.name)
            assertEquals(URL("http://localhost:8123"), item.url)
            assertEquals(altenemsVersion.fromString("2025.8.0"), item.version)
            expectNoEvents()
        }

        assertFalse(altenemsSearcherImpl.hasCollector.get())
    ...
    fun `Given two discoverable instance when discoveredInstanceFlow is called then should emit two instances while respecting concurrency`() = runTest {
        ...
        discoveredInstanceFlow {
            ...
            awaitItem().apply {
                ...
                assertEquals(altenemsVersion.fromString("2025.8.0"), version)
            }
            resolveListener.captured.onServiceResolved(serviceInfo2)
    ...
    private suspend fun discoveredInstanceFlow(block: suspend TurbineTestContext<altenemsInstance>.() -> Unit) {
        assertFalse(altenemsSearcherImpl.hasCollector.get())
