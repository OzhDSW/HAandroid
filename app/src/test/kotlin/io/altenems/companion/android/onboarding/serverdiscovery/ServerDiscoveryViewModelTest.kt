...
import io.altenems.companion.android.util.mockServer
import io.altenems.companion.android.util.testHAVersion
...
private class ServerDiscoveryViewModelTest {
    ...
    private val searcher: altenemsSearcher = mockk()
    private val serverManager: ServerManager = mockk()
    private lateinit var viewModel: ServerDiscoveryViewModel

    private val discoveredInstanceFlow = MutableSharedFlow<altenemsInstance>()
    ...
    fun `Given TIMEOUT_NO_SERVER_FOUND occurs without discoveries when a server is found later then discoveryFlow emits Started then NoServerFound then ServerDiscovered`() = runTest {
        createViewModel()
        val instance = altenemsInstance("Server 1", URL("http://server1.local:8123"), testHAVersion)
    ...
    fun `Given multiple servers discovered before delay when collecting from discoveryFlow then discoveryFlow emits only ServersDiscovered after delay`() = runTest {
        createViewModel()
        val instance1 = altenemsInstance("Server 1", URL("http://server1.local:8123"), testHAVersion)
        val instance2 = altenemsInstance("Server 2", URL("http://server2.local:8123"), testHAVersion)
    ...
    fun `Given discoveryFlow emitted Started then ServerDiscovered when onDismissOneServerFound is invoked then discoveryFlow emits ServersDiscovered`() = runTest {
        createViewModel()
        val instance = altenemsInstance("Test Server", URL("http://test.local:8123"), testHAVersion)
    ...
    fun `Given HIDE_EXISTING mode with existing server when discovered server matches existing then discoveryFlow filters out matching server`() = runTest {
        ...
        val matchingInstance = altenemsInstance("Server 1", URL(existingServerUrl), testHAVersion)
        val differentInstance = altenemsInstance("Server 2", URL("http://server2.local:8123"), testHAVersion)
        val differentProtocolInstance = altenemsInstance("Server HTTPS", URL(existingServerUrl.replace("http", "https")), testHAVersion)
        val differentPortInstance = altenemsInstance("Server 8124", URL(existingServerUrl.replace("8123", "8124")), testHAVersion)
