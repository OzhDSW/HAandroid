 ...
 * in [io.altenems.companion.android.util.HAWebViewClientTest] to avoid duplication.
 */
...
class ConnectionViewModelTest {
    ...
    @ParameterizedTest
    @ValueSource(strings = ["http://altenems.local:8123", "https://cloud.ui.nabu.casa"])
    fun `Given a valid http url when buildAuthUrl then urlFlow emits correct auth url and isLoading is false`(baseUrl: String) = runTest {
        ...
        turbineScope {
            ...
            val expectedAuthUrl = "$baseUrl/auth/authorize?response_type=code&client_id=${AuthenticationService.CLIENT_ID}&redirect_uri=altenems://auth-callback"
            advanceUntilIdle()
    ...
    fun `Given auth callback uri with code when shouldRedirect then emits Authenticated event with mTLS status and returns true`(requireMTLS: Boolean) = runTest {
        ...
        val stringUri = mockAuthCodeUri(scheme = "altenems", host = "auth-callback", authCode = authCode)

        val viewModel = ConnectionViewModel("http://altenems.local:8123", webViewClientFactory, connectivityCheckRepository, fileChooserManager)
        ...
        turbineScope {
            ...
            assertEquals("http://altenems.local:8123", event.url)
    ...
    fun `Given auth callback uri without code when shouldRedirect then no event and returns false`() = runTest {
        val stringUri = mockAuthCodeUri(scheme = "altenems", host = "auth-callback", authCode = null)
    ...
    fun `Given an error occurs when onReceivedError is invoked then runChecks is triggered`() = runTest {
        // Given
        val rawUrl = "http://altenems.local:8123"
